...
class Traversal(object):
    ...
    def promise(self, cb=None):
        future_traversers = self.traversal_strategies.apply_async_strategies(self)
        future = future_traversers.__class__()
...
class TraversalStrategies(object):
    ...
    def apply_async_strategies(self, traversal):
        # This will obviously need some work
        for traversal_strategy in self.traversal_strategies:
            # There is only a remote strategy configured here
            return traversal_strategy.apply_async(traversal)
    def __repr__(self):
